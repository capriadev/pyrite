...
- Spec 013, alcance al implementar: la spec deja la seccion en solo lectura mientras hay job abierto (una fila creada o editada en ese lapso quedaria cifrada con la clave vieja) y registra la alternativa descartada (re-escenar en el apply). Nunca se implemento rotacion, asi que si el usuario prefiere la otra via se corrige en planificacion.
