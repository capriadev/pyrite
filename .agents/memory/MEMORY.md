...
- Spec 013, escrituras durante el staging: la seccion sigue legible (y editable) con la passphrase vieja mientras el job corre, asi que una fila creada o editada en ese lapso no estaria en el set de staged y el apply la dejaria cifrada con la clave vieja (ilegible). Opciones: bloquear escrituras de la seccion mientras hay job abierto (409) o re-escenar en el apply las filas creadas/modificadas desde el snapshot. Bloquea la implementacion.
...
- Counts: `counts.service.ts` en 442 lineas (el mas grande del backend; notes 270). La spec 013 ya contempla extraer las primitivas de clave por seccion a un modulo compartido y separar las auditorias; todavia sin ejecutar.
